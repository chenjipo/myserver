...
class ModOptRetainReport extends Model{
    ...
    public function getData($request = [])
    {
         ...
         * retain
         */
        $sql_retain = "select `date`,a.`login_date`,sum(a.`login`) `retain`  
        ...
        $sql_retain = $sql_retain. $and . " group by `date`,`login_date` order by null";

        $dataBase = $this->query($sql_base,$param);
        $dataRetain = $this->query($sql_retain,$param);

        $base = $this->mergeLongData($dataBase,$dataRetain,['date'],'login_date','retain',$request);
