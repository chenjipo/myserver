...
namespace YXLib\foundation\ipapi;
...
class IpSearch {
    ...
    function __construct() {
		...
		$this->database=dirname(__FILE__).'/data/qqzengip.dat';

        $this->fp = @fopen($this->database, 'rb');
     ...
     * @return <type>
     */
    function getIpAddr($ip = '0.0.0.0')
    {
        $ip_addr = $this->getAddr($ip);
        //string(75) "亚洲|中国|广东|广州|荔湾|电信|440103|China|CN|113.2442|23.12592"
        //转成数组
        $ip_addr = explode('|', $ip_addr);
        return array('province' => $ip_addr[2], 'city' => $ip_addr[3] . '-' . $ip_addr[4] . '-' . $ip_addr[5], 'country' => $ip_addr[1], 'continent' => $ip_addr[0]);
    }
